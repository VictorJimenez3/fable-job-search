...
module.exports = async (req, res) => {
  ...
  const { action, id, ids, url, company, title, location, vote, reason, stage } = req.body || {};
  const manual = action === "manual-add";
  const research = action === "research-company";
  if (!["track", "applied", "stage", "untrack", "manual-add", "research-company", "feedback", "archive"].includes(action)
  ...
  const r = await gh(`/repos/${REPO}/dispatches`, s.g, {
    ...
    body: JSON.stringify({
      ...
      client_payload: { action, id, ids: researchIds, url, company, title, location,
        vote, reason, stage, profile: PROFILE },
    }),
  });

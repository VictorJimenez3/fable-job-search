...
module.exports = async (req, res) => {
  ...
  const { action, id, ids, url, company, title, location, vote, reason } = req.body || {};
  const manual = action === "manual-add";
  const research = action === "research-company";
  if (!["track", "applied", "untrack", "manual-add", "research-company", "feedback", "archive"].includes(action)

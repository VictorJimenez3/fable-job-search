// POST {action: "track"|"applied"|"stage"|"untrack"|"manual-add"|"research-company"|"score-preferences", ...}
// → repository_dispatch.
...
module.exports = async (req, res) => {
  ...
  const notification = action === "notification-preference";
  const lane = normalizeProfile(profile || PROFILE);
  if (!VALID_PROFILES.has(lane) || (lane === "cheme" && PROFILE !== "cheme")) {
    res.status(400).json({ error: "unsupported radar profile" });
    return;
  }
  if (!["track", "applied", "stage", "untrack", "manual-add", "research-company", "feedback", "archive", "score-preferences", "notification-preference"].includes(action)
      || (scorePreferences && (!preferences || typeof preferences !== "object"))
      || (notification && (!["new_grad_email", "internship_email"].includes(key) || typeof enabled !== "boolean"))
      || ((!scorePreferences && !notification) && (!manual && !id))
      || (manual && (!company || !title || !url))) {

// POST {action: "track"|"applied"|"stage"|"untrack"|"manual-add"|"research-company", ...}
// → repository_dispatch.
...
module.exports = async (req, res) => {
  ...
  if (!["track", "applied", "stage", "untrack", "manual-add", "research-company", "feedback", "archive", "notification-preference"].includes(action)
      || (notification && (!["new_grad_email", "internship_email"].includes(key) || typeof enabled !== "boolean"))
      || (!notification && (!manual && !id) || (manual && (!company || !title || !url)))) {
  ...
  const r = await gh(`/repos/${REPO}/dispatches`, s.g, {
    ...
    body: JSON.stringify({
      ...
      client_payload: { action, id, ids: researchIds, url, company, title, location,
        vote, reason, stage, profile: lane, key, enabled },
    }),
  });

...
module.exports = async (req, res) => {
  ...
  const { action, id, ids, url, company, title, location, vote, reason, stage, preferences } = req.body || {};
  const manual = action === "manual-add";
  const research = action === "research-company";
  const scorePreferences = action === "score-preferences";
  if (!["track", "applied", "stage", "untrack", "manual-add", "research-company", "feedback", "archive", "score-preferences"].includes(action)
      || (scorePreferences && (!preferences || typeof preferences !== "object"))
      || (!manual && !scorePreferences && !id) || (manual && (!company || !title || !url))) {
  ...
  const r = await gh(`/repos/${REPO}/dispatches`, s.g, {
    ...
    body: JSON.stringify({
      ...
      client_payload: { action, id, ids: researchIds, url, company, title, location,
        vote, reason, stage, preferences, profile: PROFILE },
    }),
  });

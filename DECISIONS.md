...
## 36. Response-rate analytics: one canonical breakdown, not two (2026-07-14)
...
The memo previously had two separate per-sector response-rate computations
living in different places (an inline all-time one inside the Funnel section,
...
were consequently outside the US. Rules v6 recognizes common ISO-3 codes,
additional country/city evidence (including Bintulu, Malaysia), and explicit
graduate-only titles and school-specific `ONLY` postings that are ineligible
for a general undergraduate search. A multi-location posting
